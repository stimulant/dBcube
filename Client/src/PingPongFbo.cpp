...
: mCurrentFbo(0)
{
	...
	unsigned int max =gl::Fbo::getMaxAttachments();
...
void PingPongFbo::bindFramebuffer(bool currentFbo)
{
    mFbos[ currentFbo ].bindFramebuffer();
}

void PingPongFbo::unbindFramebuffer(bool currentFbo)
{
    mFbos[ currentFbo ].unbindFramebuffer();
}
