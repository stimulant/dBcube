...
void main()
{
	...
	gl_FragColor.rgb	= vec3( cubeDiff * ppEyeDiff * vColor.rgb + cubeSpec * pow( 1.0 - ppEyeDiff, 2.0 ) )
						+ ppEyeFres * yPer + vColor.rgb + distFromCeilingShine * vColor.rgb - ( 0.2 - ( vNormal.y * 0.5 + 0.5 ) * 0.2 );
	gl_FragColor.a		= vColor.a * 0.4;
	//gl_FragColor.rgb = vec3(bug);
}